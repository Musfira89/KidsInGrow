...
export const generateAndSaveReport = async (req, res) => {
  ...
  try {
    ...
    const areaX = 50;
    const totalScoreX = 200;
    const scoreInterpretationX = 350;
    const columnWidth = 150; // Define column width for alignment
    const headerY = doc.y;

    // Add table headers
    doc.fontSize(14).text('Area', areaX, headerY, { underline: true })
      .text('Total Score', totalScoreX, headerY, { underline: true })
      .text('Score Interpretation', scoreInterpretationX, headerY, { underline: true });
    doc.moveDown();
    ...
    responseData.forEach(response => {
      ...
      doc.fontSize(12).text(category, areaX, currentY);
      doc.fontSize(12).text(totalMarks.toString(), totalScoreX, currentY);
      ...
      doc.fontSize(12).fillColor(color).text(interpretation, scoreInterpretationX, currentY);
      
      // Update Y-position for the next category
      currentY += 20; // Adjust the number to control vertical spacing
